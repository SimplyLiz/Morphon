...
This is the analytical core of the paper. We document six failure modes encountered during development of MI. For each, we describe (i) the symptom, (ii) the biological parallel, (iii) the fix derived from biology, and (iv) the empirical impact of the fix.
...
\subsection{Novelty Gain Collapse}
\label{sec:novelty_gain_collapse}

\paragraph{Symptom.}
During MNIST training, the novelty gain channel \texttt{ng} (acetylcholine modulation, which scales three-factor STDP weight updates) converges to $\approx$0.32 rather than remaining near 1.0. Three-factor STDP is throttled mid-training. Accuracy stagnates despite healthy Endoquilibrium stage cycling and adequate morphon activity. Observable as \texttt{ng=0.32} in the Endoquilibrium summary line printed during training, typically after 500--1000 images.

\paragraph{Diagnosis.}
Endoquilibrium Rule 7 (energy pressure) executed \texttt{ng $\times$= 0.2} whenever morphon energy utilization exceeded 0.85. During dense MNIST training, energy pressure is near-constant: morphons fire on nearly every image, exhausting metabolic reserves. The slow EMA of the suppressed value converges to $\sim$0.32 over training, occasionally refreshed above 0.85. The net effect: the ACh channel that gates STDP effectiveness is reduced to 32\% of its design value, throttling learning precisely when the system is most active.

The design error is a category mismatch: energy pressure should express via \texttt{plasticity\_mult} (already done by Rule 7) not via neuromodulatory channel amplitude. Suppressing \texttt{ng} conflates two distinct mechanisms - metabolic gating (slow timescale, reduces weight change magnitude) and attentional modulation (fast timescale, ACh responds to novel stimuli). They are not interchangeable.

\paragraph{Biological parallel.}
Acetylcholine release in the cortex \emph{increases} under metabolic stress - heightened arousal and attention are the physiological response to an energy crisis, not suppression of the learning signal \cite{hasselmo1995neuromodulation}. Energy conservation in biology is expressed via spike-rate reduction and metabolic downregulation, not by silencing the neuromodulatory learning gate.

\paragraph{Fix.}
Remove \texttt{ng} modification from Rule 7's energy\_emergency and energy\_critical paths. Only \texttt{plasticity\_mult} is suppressed under energy pressure. The \texttt{ng} channel responds purely to novelty and stimulus salience as intended.

\paragraph{Result.}
\texttt{ng} holds at \textbf{1.60} throughout MNIST training (was: 0.32 after early epochs). CartPole standard run: \texttt{ng} = \textbf{1.40} throughout. The fix is confirmed independently on both tasks. On MNIST, the primary accuracy gain came from stateless training (Section~\ref{sec:stateless_training}); the novelty-gain fix contributes modulatory stability rather than classification accuracy (the ablation, §\ref{sec:stateless_training}, confirms the two fixes are independent).

\paragraph{Diagnostic.}
\emph{Endoquilibrium \texttt{ng} channel}: \texttt{ng} $\leq 0.5$ during active training is the signature. A healthy \texttt{ng} should fluctuate around 1.0--2.0. Persistent \texttt{ng} $\approx 0.32$ during dense training indicates energy-pressure suppression; inspect Rule 7 logic. The diagnostic is cheap: the Endo summary line prints \texttt{ng} on every medium tick.

\paragraph{Implications beyond MI.}
Any neuromodulatory architecture that combines energy-pressure homeostasis with a learning-rate modulation channel risks this failure when the two are implemented in the same regulatory path. The fix is to enforce separation: metabolic gating modifies weight-change magnitude or firing thresholds; neuromodulatory channels respond only to their designated signals. Conflating them creates a training dynamic where the system becomes less plastic exactly when it needs to learn most - a failure mode that is silent in the loss curve (no divergence, no obvious error) and detectable only via channel-level logging.

\subsection{Diagnostic Instruments: Consolidated Reference}
\label{sec:diagnostics}

Each failure mode above was discovered through a specific measurement introduced alongside it. We consolidate the six instruments here for reference.
...
\begin{description}
  ...
  \item[\textbf{Endoquilibrium summary line.}] \texttt{pm} at 0.6 with stage = Mature on a fresh task: premature throttling (Section~\ref{sec:premature_mature}). \texttt{ng} $\leq 0.5$ during active training: novelty gain collapse (Section~\ref{sec:novelty_gain_collapse}).
  ...
  \item[\textbf{Post-hoc neuron labeling.}] Assign each associative morphon the class it fires most frequently on labeled test inputs. $\leq$2 labeled classes by epoch 2 predicts iSTDP timescale mismatch. All 10 classes represented with $\geq$2 morphons each is the healthy target. Described in Section~\ref{sec:istdp_timescale}.
  \item[\textbf{Sequential vs.\ shuffled accuracy gap.}] Evaluate the same test set in natural order and in randomly shuffled order. Gap $>$5pp indicates co-adaptation to sequential context (Section~\ref{sec:stateless_training}). Healthy per-image classifier: gap $<$3pp.
\end{description}
