...
<ul>
  ...
  <li>On MNIST, the system reaches 30% intact accuracy and <strong>48% after destroying 30% of associative neurons and allowing the network to regrow</strong> &mdash; a self-healing response that exceeds the intact baseline by 18 percentage points.</li>
  <li>On a 4-tier NLP readiness benchmark, the morphon substrate produces representations that an analog readout extracts at 99% / 62% / 85% across the first three tiers, even though the spike-based output pathway loses the same information.</li>
</ul>
