...
fn train_and_eval(
    ...
    for (bi, &idx) in indices.iter().take(n_train).enumerate() {
        ...
        system.inject_novelty(0.1);
        system.train_readout(train_labels[idx], READOUT_LR);

        if (bi + 1) % 100 == 0 {
            let acc = evaluate(system, test_images, test_labels, 50);
            let elapsed = start.elapsed().as_secs();
            // Check output diversity: read outputs for current image
            let outs = system.read_output();
            let o_min = outs.iter().cloned().fold(f64::INFINITY, f64::min);
            let o_max = outs.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
            eprintln!("  [{}] {:>4}/{} acc={:.1}% out=[{:.3}..{:.3}] ({:.0}s)",
                label, bi + 1, n_train, acc, o_min, o_max, elapsed);
        }
    }
...
fn main() {
    ...
    let (n_train, n_test, recovery_n) = match profile {
        "extended" => (10000, 500, 3000),
        "standard" => (5000, 300, 1500),
        _ => (3000, 200, 500),
    };

    eprintln!("=== MORPHON MNIST V2 — Supervised Readout [{}] ===\n", profile);
    ...
    let base_acc = train_and_eval(&mut base, &train_images, &train_labels,
        &test_images, &test_labels, n_train, "BASE", &mut rng_b);
    eprintln!("  Baseline: {:.1}% ({:.0}s)\n", base_acc, t0.elapsed().as_secs_f64());
    ...
    let v2_acc = train_and_eval(&mut v2, &train_images, &train_labels,
        &test_images, &test_labels, n_train, "V2  ", &mut rng_v);
    ...
    let recovery_acc = train_and_eval(&mut v2, &train_images, &train_labels,
        &test_images, &test_labels, recovery_n, "RECV", &mut rng_v);
